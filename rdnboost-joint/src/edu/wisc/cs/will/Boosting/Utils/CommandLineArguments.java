...
public class CommandLineArguments {
	...
	public static final String teacherModel = "teacherModel";
	private String teacherDirVal = null;
    
    public static final String seedFlag = "seed";
    private long seedVal = -1;
	...
	public static final String piFlag = "pi";
	private boolean usePI = false;
	
	public static final String piLambdaFlag = "piLambda";
	private double piLambda = 1.0;
	...
	public double getPiLambda() {
		return piLambda;
	}
	
	public void setPiLambda(double piLambda) {
		this.piLambda = piLambda;
	}
	...
	public boolean parseArgs(String[] args) {
		for (int i = 0; i < args.length; i++) {		
			...
			if (argMatches(args[i], teacherModel)) {
				setTeacherDirVal(args[++i]);
				continue; 
			}
			if (argMatches(args[i], teacherModel)) {
				setTeacherDirVal(args[++i]);
            ...
            if (argMatches(args[i], seedFlag)) {
                if (isArgumentNotAFlag(args, i+1)) {
                    setSeedVal(Long.parseLong(args[++i]));
                }
                continue;
            }
			...
			if (argMatches(args[i], piFlag)) {
				usePI = true;
				continue;
			}
			if (argMatches(args[i], piLambdaFlag)) {
				piLambda = Double.parseDouble(args[++i]);
				continue;
			}
	...
	public static String getUsageString() {
        ...
        result += argPrefix + seedFlag + " <Seed>: Random seed for reproducibility.\n";
        
        result += argPrefix + teacherModel + " <Teacher Directory>: Directory containing the teacher model for joint training.\n";
	...
	public String getTeacherDirVal() {
		return teacherDirVal;
	}

	public void setTeacherDirVal(String teacherDirVal) {
		this.teacherDirVal = teacherDirVal;
	}
    ...
    public void setSeedVal(long seedVal) {
