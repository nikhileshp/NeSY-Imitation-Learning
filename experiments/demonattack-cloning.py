...
"""
DemonAttack Cloning with RDN-style Data Loading
================================================
...
Actions: NOOP (0), FIRE (1), RIGHT (3), LEFT (4)
"""
    ...
    with open(facts_file, 'r') as f:
        for line in f:
            ...
            if match:
                ...
                # e.g., visibleenemy(s1, enemy_0) -> visibleenemy_enemy_0
                if len(args) == 1:
                    feature = predicate
                else:
                    # Join remaining args
...
def train_and_evaluate(action, seed, save_dir, negpos_ratio, debug=False):
    ...
    # Structure: rdn_models/demonattack/all/negpos_{NEGPOS}_mlp_64_32_bc/{action}/seed_{seed}/
    experiment_dir = os.path.join(save_dir, f"negpos_{int(negpos_ratio)}_mlp_64_32_bc", action, f"seed_{seed}")
    os.makedirs(experiment_dir, exist_ok=True)
    
    # --- Training Data Loading ---
    print("Loading Training Data...", flush=True)
    train_facts_file = f"data/demonattack/all/{action}/train/train_facts.txt"
    train_pos_file = f"data/demonattack/all/{action}/train/train_pos.txt"
    train_neg_file = f"data/demonattack/all/{action}/train/train_neg.txt"
    ...
    clf = MLPClassifier(
        ...
        max_iter=200 if not debug else 10,
        random_state=seed,
        verbose=True
    )
    ...
    # Save feature names
    features_path = os.path.join(experiment_dir, "features.txt")
    with open(features_path, 'w') as f:
        for feat in unique_features:
            f.write(feat + '\n')
    print(f"Feature names saved to {features_path}", flush=True)
    
    # --- Testing ---
    print("Loading Test Data...", flush=True)
    test_facts_file = f"data/demonattack/all/{action}/test/test_facts.txt"
    test_pos_file = f"data/demonattack/all/{action}/test/test_pos.txt"
    test_neg_file = f"data/demonattack/all/{action}/test/test_neg.txt"
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="rdn_models/demonattack/all", help="Directory to save models and logs")
    ...
    print("\n" + "="*80)
    print("DemonAttack Behavior Cloning - RDN-style Data Loading")
    print("="*80)
    print(f"Seed: {args.seed}")
    print(f"Actions: {', '.join(actions_to_run)}")
    print(f"NegPos Ratios: {ratios_to_run}")
