"""
Process multiple DemonAttack trajectories with DEBUG logging.
"""
import os
import pandas as pd
import numpy as np
...
from env.demonattack.object_detector import DemonAttackObjectDetector
...
def detect_objects_in_frame(detector, frame_path, debug=False):
    ...
    Returns:
        tuple: (detected_objects_list, bboxes_list, relationships_str)
    """
    ...
    if debug:
        print(f"  DEBUG: Final count: {len(objects_list)} objects")
    
    # Analyze relationships
    analyzer = DemonAttackRelationshipAnalyzer()
...
def process_trajectory(data_folder, txt_file, detector, trajectory_id, debug_frames=5):
    ...
    if len(df) > 0:
        ...
        # DemonAttack frames typically use full qframe_id as filename
        test_patterns = [
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc=f"  {trajectory_id}"):
        ...
        # Construct frame filename - use the full qframe_id
...
def process_all_trajectories(base_path, output_file='combined_demonattack_data.txt'):
    """
    Process all trajectories and create combined dataframe.
    ...
    for item in os.listdir(base_path):
        ...
        if os.path.isdir(item_path) and ('RZ_' in item or 'JAW_' in item or 'LG_' in item):
    ...
    total_relationships = sum(1 for x in combined_df['relationships'] if x and x.strip())
    print(f"    Frames with relationships: {total_relationships}")
    print(f"    Frames without relationships: {len(combined_df) - total_relationships}")
...
if __name__ == "__main__":
    ...
    output_file = sys.argv[2] if len(sys.argv) > 2 else "combined_demonattack_data.txt"
    
    print(f"\n{'='*70}")
    print("DemonAttack Multi-Trajectory Object Detection Processor (DEBUG)")
    print(f"{'='*70}")
