"""
Preprocess DemonAttack relationship data for learning.
"""
import pandas as pd
...
# DemonAttack-specific modes (relationship predicates)
modes = [
    "visibleEnemy(+state, -enemy).",
    "visibleEnemyMissile(+state, -emissile).",
    "visiblePlayerMissile(+state, -pmissile).",
    "enemyDirectlyAbove(+state, +enemy).",
    "enemyMissileDirectlyAbove(+state, +emissile).",
    "leftOfMissile(+state, +emissile).",
    "rightOfMissile(+state, +emissile).",
    "leftOfEnemy(+state, +enemy).",
    "rightOfEnemy(+state, +enemy).",
    "nearbyMissile(+state, +emissile).",
    "fireReady(+state,+pmissile).",
    "action(+state, #name)."
]

# Bridgers for DemonAttack (if needed)
bridgers = ["visibleEnemy/2", "visibleEnemyMissile/2", "visiblePlayerMissile/2"]

# DemonAttack action space (6 actions: NOOP=0, FIRE=1, RIGHT=2, LEFT=3, RIGHTFIRE=4, LEFTFIRE=5)
primitive_actions = ["noop", "fire", "right", "left"]
...
# Argument parser
parser = argparse.ArgumentParser(description="Process DemonAttack relationship file")
parser.add_argument("--file", type=str, default="", help="Relationship file (combined_demonattack_data.txt)")
...
if args.all:
    ...
    base_dir = f"data/demonattack/single_t/{traj_id}"
...
# Filter to valid DemonAttack actions only (0-5)
