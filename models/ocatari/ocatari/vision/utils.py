...
# Side Facing
def facing_side(image: np.ndarray, colors:List[List[int]], coords_list: List[Tuple[int, int, int, int]]) -> List[str]:

    # Get the array of the object in the image using coords_list and get the midpoint of the object and count number of pixels of the colors on each side
    if not coords_list:
        return []
    for i, coords in enumerate(coords_list):
        ...
        # For each color of the object count the number of pixels on the left and right side
        for color in colors:
            mask = None
            obj_array = image[y:y+h, x:x+w]
            mask = cv2.inRange(obj_array, np.array(color), np.array(color))
            if mask is None:
                continue
            obj_left_side = mask[:, :mask.shape[1] // 2]
            obj_right_side = mask[:, mask.shape[1] // 2:]
            left_count += np.sum(obj_left_side)
            right_count += np.sum(obj_right_side)
...
#I need to make color accept a list of colors, not just one color
def find_objects(image, colors, size=None, tol_s=10,
    ...
    for color in colors:
        ...
        contour, _ = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, 1)
        contourList.extend(contour)
    contours = [cv2.boundingRect(cnt) for cnt in contourList]
    ...
    for cnt in contours:
        ...
        #Check for overlapping objects and combine them
